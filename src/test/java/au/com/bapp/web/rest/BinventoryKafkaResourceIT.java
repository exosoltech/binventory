...
import au.com.bapp.config.KafkaProperties;
...
class BinventoryKafkaResourceIT {
    ...
    void setup() {
        ...
        BinventoryKafkaResource kafkaResource = new BinventoryKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/binventory-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/binventory-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
